...
use generic_array::typenum::{self};
...
fn test_extract_all<Tree: 'static + MerkleTreeTrait>() {
    ...
    let data: Vec<u8> = (0..nodes)
        .flat_map(|_| {
            let v: <Tree::Hasher as Hasher>::Domain = <Tree::Hasher as Hasher>::Domain::random(rng);
            v.into_bytes()
        })
        .collect();
...
fn test_generate_labels_aux(
    ...
    let graph =
        StackedBucketGraph::<PoseidonHasher>::new(None, nodes, BASE_DEGREE, EXP_DEGREE, porep_id)
            .unwrap();
